...
# Load Packages (more than needed)
library(tidyverse)
library(plotrix)
library(lme4)
library(car)
...
urb_perc_base <- read.csv("data/wb_urb_percent.csv", header = T, sep = ",")
fexp_base <- read.csv("data/wb_food_export_perc.csv", header = T, sep = ",")
...
Urb_Exp <- left_join(Urb_perc,Food_exp) %>% 
  rename(Country = ï..Country.Name) %>%  # idk why Country.Name is imported so weird
  filter(!is.na(food_exp_percent), !is.na(urb_percent))
View(Urb_Exp)
head(Urb_Exp)


# Testing with some larger country groups
Big_Ones <- Urb_Exp %>% 
